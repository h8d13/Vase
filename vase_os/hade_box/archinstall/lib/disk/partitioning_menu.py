...
class PartitioningList(ListManager[DiskSegment]):
	def __init__(
		...
		prompt = ('Partition management: {}').format(device.device_info.path) + '\n'
		prompt += ('Total length: {}').format(device.device_info.total_size.format_size(Unit.MiB))
		self._info = prompt + '\n'
	...
	def _prompt_size(self, free_space: FreeSpace) -> Size:
		...
		text = ('Selected free space segment on device {}:').format(device_info.path) + '\n\n'
		...
		prompt += ('Size: {} / {}').format(max_sectors, max_bytes) + '\n\n'
		...
		title = ('Size (default: {}): ').format(max_size.format_highest())
