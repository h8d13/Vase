...
class Installer:
	...
	def _write_environment_file(self, env_vars: dict[str, str]) -> None:
		...
		# Read existing content if file exists # Preservers original headers
		existing_content = ''
		if env_file.exists():
		...
		new_lines = [
			...
			'# Generated by vase/archinstall for graphics optimization'
		]
		...
		if len(new_lines) > 3:  # More than just the original comments
			# Append to file
	...
	def _add_grub_bootloader(
		...
		if self._gfx_driver and self._gfx_driver.is_nvidia():
			...
			# Build the hardware-specific parameters string for bootloader
			hw_params = []
			if self._gfx_driver.is_nvidia():
			...
			if self._gfx_driver == GfxDriver.NvidiaProprietary or GfxDriver.NvidiaOpenSource:
				...
				#hw_params.append('nvidia.NVreg_PreserveVideoMemoryAllocations=1')
				# This would be better in /etc/mkinitcpio.conf # i915.modeset=1
